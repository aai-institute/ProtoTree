...
for i in range(len(tableau_colorblind_own_4)):
    ...
    tableau_colorblind_own_4[i] = (r / 255.0, g / 255.0, b / 255.0)
# Vibrant qualitative colour scheme from https://personal.sron.nl/~pault/
paultol_palette_colorblind = [
    (0, 119, 187),
    (0, 153, 136),
    (238, 119, 51),
    (204, 51, 17),
...
for i in range(len(paultol_palette_colorblind)):
    ...
    paultol_palette_colorblind[i] = (r / 255.0, g / 255.0, b / 255.0)
...
for rank, d in enumerate(datasets):
    ...
    plt.fill_between(
        range(len(mean)), mean - std, mean + std, color=chosen_palette[rank], alpha=0.3
    )
    plt.plot(range(len(mean)), mean, lw=2.5, color=chosen_palette[rank], marker="o")
    min_height = data[data["dataset"] == d]["min_height"].values[0]
    plt.axvline(
        x=min_height - 1,
        linewidth=1.7,
        linestyle="dotted",
        color=chosen_palette[rank],
        alpha=0.9,
    )

    ensemble_acc = data[data["dataset"] == d]["ensemble_acc"].values[0]
    plt.plot(
        range(len(ensemble_acc)),
        ensemble_acc,
        lw=2.5,
        color=chosen_palette[rank],
        marker="d",
