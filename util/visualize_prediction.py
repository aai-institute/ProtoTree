...
def upsample_local(
    ...
    if not os.path.exists(dir):
        os.makedirs(dir)
    with torch.no_grad():
        distances_batch = tree.prototype_distances_to_patches(sample)
    ...
    for i, node in enumerate(decision_path[:-1]):
        ...
        plt.imsave(
            fname=os.path.join(
                dir, "%s_heatmap_latent_similaritymap.png" % str(decision_node_idx)
            ),
...
# TODO: fix this, all path things are wrong
def gen_pred_vis(
    tree: ProtoTree,
    sample: torch.Tensor,
    sample_dir: str,
    ...
    upsample_path = os.path.join(
        os.path.join(args.log_dir, args.dir_for_saving_images), "pruned_and_projected"
    )
    ...
    idx2node = tree.idx2node
    leaf = idx2node[leaf_ix]
    decision_path = leaf.get_path_from_ancestor()
