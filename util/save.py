...
def save_tree(
    ...
    torch.save(
        optimizer.state_dict(), f"{log.checkpoint_dir}/latest/optimizer_state.pth"
    )
    torch.save(
        scheduler.state_dict(), f"{log.checkpoint_dir}/latest/scheduler_state.pth"
    )
...
def save_best_train_tree(
    ...
    if train_acc > best_train_acc:
        ...
        torch.save(
            optimizer.state_dict(),
            f"{log.checkpoint_dir}/best_train_model/optimizer_state.pth",
        )
        torch.save(
            scheduler.state_dict(),
            f"{log.checkpoint_dir}/best_train_model/scheduler_state.pth",
        )
