...
def test_model(config: dict):
    ...
    match config["model_type"]:
        case "protopnet":
            model = ProtoPNet.load_from_checkpoint(
                config["model_checkpoint"], map_location=torch.device(device)
            )
        case "prototree":
            model = ProtoTree.load_from_checkpoint(
                config["model_checkpoint"], map_location=torch.device(device)
            )
        case _:
    ...
    match (model_type):
        ...
        case "prototree":
            ...
            if explain_proto:
                pruned_acc, local_score_expl = eval_model(
                    model, test_loader, explain=True
                )
            else:
    ...
    match (model_type):
        case "prototree":
            ...
            save_decision_flow_visualizations(
                ...
                prototypes_info,
                vis_dir / "explanations",
                patches_dir=patches_dir,
                scores=local_score_expl,
                img_size=img_size,
            )
        case "protopnet":
            assert (
                local_score_expl is not None
            ), "Saving top k prototypes works only with local scores previously computation"
            save_top_k_prototypes(
                prototypes_info=prototypes_info,
                scores=local_score_expl,
