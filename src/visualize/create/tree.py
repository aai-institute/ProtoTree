...
from src.visualize.create.dot import FONT, _node_name, gen_leaf, graph_with_components
...
def _gen_internal_node_img(
    proto_idx: int, prototypes_info: dict, global_scores_info: pd.DataFrame
) -> Image:
    proto_info = prototypes_info[str(proto_idx)]

    img_orig = Image.open(proto_info["path"])
    bb = proto_info["bbox"]
    patch_img_orig = img_orig.crop(
        bb
    )  # TODOs gio check PIL bbox as needs to be done x,y,w,h, or x1,y1,x2,y2img_orig[bb[1]:bb[3], bb[0]:bb[2]]
    ...
    if global_scores_info is not None:
        proto_scores = global_scores_info.loc[
            global_scores_info["prototype"] == proto_idx
        ].set_index("prototype")
    ...
    if global_scores_info is None:
        (
            together_w,
            together_h,
        ) = w + INTERNAL_NODE_IMG_GAP + wbb + INTERNAL_NODE_IMG_GAP, max(h, hbb)
    else:
        (
            together_w,
            together_h,
        ) = w + INTERNAL_NODE_IMG_GAP + wbb + INTERNAL_NODE_IMG_GAP + wscore, max(
            h, hbb, hscore
        )
    ...
    if global_scores_info is not None:
        together.paste(
            img_scores, (w + INTERNAL_NODE_IMG_GAP + wbb + INTERNAL_NODE_IMG_GAP, 0)
        )
    return together.convert("RGB")
