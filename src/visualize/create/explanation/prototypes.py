...
def save_top_k_prototypes(prototypes_info: dict[str, dict], scores: pd.DataFrame = None, top_k : int = 5, img_size : tuple = (224, 244), save: bool = True):
    ...
    for path in paths:
        ...
        for id, proto_id in enumerate(top_k_protos_id):
            ...
            proto_scores_df = round(img_scores.loc[img_scores["prototype"] == proto_id], 3)
            ...
            if plot_scores:
                ...
                proto_scores_df.plot(x="modification", y="delta", ax=axs[1][id], kind="bar", legend=False)
...
def save_prototypes(proto_info: dict[str, dict], global_expl: pd.DataFrame = None, img_size : tuple =(224, 224)):
    """
    ...
    for proto_id, proto in proto_info.items():
        ...
        img = cv2.cvtColor(cv2.resize(cv2.imread(proto["path"]), img_size), cv2.COLOR_BGR2RGB) 
        img = img / 255
        patch = img[proto["bbox"][1]:proto["bbox"][3], proto["bbox"][0]:proto["bbox"][2]]
        bbox_inds = patches.BboxInds(w_low=proto["bbox"][0],
                                    h_low=proto["bbox"][1],
                                    w_high=proto["bbox"][2],
                                    h_high=proto["bbox"][3]
        )
