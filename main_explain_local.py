...
def get_local_expl_args() -> argparse.Namespace:
    ...
    parser.add_argument("--prototree", type=str, help="Directory to trained ProtoTree")
    parser.add_argument(
        "--log_dir",
        type=str,
    ...
    parser.add_argument(
        "--image_size", type=int, default=224, help="Resize images to this size"
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not os.path.exists(
            os.path.join(os.path.join(args.log_dir, args.results_dir), class_name)
        ):
            os.makedirs(
                os.path.join(os.path.join(args.log_dir, args.results_dir), class_name)
            )
