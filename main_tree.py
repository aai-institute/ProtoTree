...
def run_tree(args=None):
    ...
    if epoch < args.epochs + 1:
        ...
        for epoch in range(epoch, args.epochs + 1):
            ...
            if tree._kontschieder_train:
                train_info = train_epoch_kontschieder(
                    ...
                    optimizer,
                    epoch,
                    args.disable_derivative_free_leaf_optim,
                    device,
                    log,
                    log_prefix,
                )
        ...
        original_test_acc = eval_info["test_accuracy"]
        best_test_acc = save_best_test_tree(
            tree, optimizer, scheduler, best_test_acc, eval_info["test_accuracy"], log
        )
        log.log_values(
    ...
    leaf_labels = analyse_leafs(
        tree, epoch + 2, len(classes), leaf_labels, args.pruning_threshold_leaves, log
    )
    analyse_leaf_distributions(tree, log)
    ...
    leaf_labels = analyse_leafs(
        tree, epoch + 3, len(classes), leaf_labels, args.pruning_threshold_leaves, log
    )
    analyse_leaf_distributions(tree, log)
    eval_info = eval(tree, testloader, name, device, log)
