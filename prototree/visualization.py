...
def save_prototype_visualizations(
    ...
    :param save_heatmaps: whether to save heatmaps displaying similarities between the prototypes and the image
        patches
    :return:
    """
    ...
    for node, patch_info in node_to_patch_info.items():
        ...
        in_closest_patch = np.uint8(similarity_map == similarity_map.max())
        in_closest_patch = latent_to_pixel(in_closest_patch)
        h_low, h_high, w_low, w_high = covering_rectangle_indices(in_closest_patch)
        ...
        if save_heatmaps:
            ...
            pixel_heatmap = _apply_color_map(pixel_heatmap)
            overlayed_original_img = 0.5 * original_image + 0.2 * pixel_heatmap
